...
def get_study():
    try:
        ...
        return redirect(url_for('get_register'))
    return render_template('study.html', user=current_user, method='GET')  
...
def get_test():
    ...
    try:
        ...
        flash("You need to select more boxes to generate a valid quiz! Selecting 3-4 should do the trick.")
        return redirect(url_for('get_quiz'))
...
def post_test():
    ...
    try:
        ...
        session['grades'] = {'current_date': datetime.today().strftime('%m-%d-%Y'), 'num_correct': score, 'num_total':len(answers)}
    else:
...
def grades():
    ...
    if(current_user.is_instructor == False):
        #student mode
        grades = Score.query.filter_by(user_email=current_user.email).all()
